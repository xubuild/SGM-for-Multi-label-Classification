...
class seq2seq(nn.Module):
    ...
    def __init__(self, config, src_vocab_size, tgt_vocab_size, use_cuda, pretrain=None, score_fn=None):
        ...
        self.encoder = models.rnn_encoder(
            config, src_vocab_size, embedding=src_embedding)
        if config.shared_vocab == False:
            self.decoder = models.rnn_decoder(
                config, tgt_vocab_size, embedding=tgt_embedding, score_fn=score_fn)
        else:
            self.decoder = models.rnn_decoder(
                config, tgt_vocab_size, embedding=self.encoder.embedding, score_fn=score_fn)
    ...
    def forward(self, src, src_len, tgt, tgt_len):
        lengths, indices = torch.sort(
            src_len.squeeze(0), dim=0, descending=True)
        ...
        outputs, final_state = self.decoder(
            tgt[:-1], state, contexts.transpose(0, 1))
    ...
    def sample(self, src, src_len):
        ...
        sample_ids, final_outputs = self.decoder.sample(
            [bos], state, contexts.transpose(0, 1))

    ...
    def forward(self, input, lengths):
        ...
        if not self.config.bidirec:
            ...
            h = h.transpose(0, 1).contiguous().view(
                batch_size, -1, 2 * self.config.encoder_hidden_size)
...
class gated_rnn_encoder(nn.Module):
    ...
    def __init__(self, config, vocab_size, embedding=None):
        ...
        self.gated = nn.Sequential(
            nn.Linear(config.encoder_hidden_size, 1), nn.Sigmoid())
    ...
    def __init__(self, config, vocab_size, embedding=None, score_fn=None):
        ...
        if self.score_fn.startswith('general'):
            ...
            self.linear_weight = nn.Linear(
                config.emb_size, config.decoder_hidden_size)
    ...
    def forward(self, inputs, init_state, contexts):
        if not self.config.global_emb:
            ...
            for i in range(max_time_step-1):
                emb1 = torch.bmm(soft_score.unsqueeze(
                    1), self.embedding.weight.expand((batch_size, a, b)))
                emb2 = embs[i+1]
                gamma = torch.sigmoid(self.gated1(emb1.squeeze()) +
    ...
    def compute_score(self, hiddens):
        if self.score_fn.startswith('general'):
            if self.score_fn.endswith('not'):
                scores = torch.matmul(self.linear(hiddens), Variable(
                    self.embedding.weight.t().data))
            else:
                scores = torch.matmul(self.linear(
                    hiddens), self.embedding.weight.t())
            ...
            if self.score_fn.endswith('not'):
                scores = torch.matmul(hiddens, Variable(
                    self.embedding.weight.t().data))
            else:
    ...
    def sample(self, input, init_state, contexts):
        ...
        for i in range(max_time_step):
            output, state, attn_weights = self.sample_one(
                inputs[i], soft_score, state, contexts, mask)
            if self.config.global_emb:
